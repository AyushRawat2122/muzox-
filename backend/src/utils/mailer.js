...
export const sendEmail = async ({ email, otp, name, type, userId }) => {
  try {
    ...
    if (type === "VERIFICATION") {
      message = "Your gateway to a premium music experience starts now. Verify your email using the OTP below:";
    ...
    const mailOptions = {
      ...
      html: `
       <div style="background: #000; padding: 50px 0; font-family: 'Montserrat', sans-serif;">
        <div style="max-width: 600px; margin: auto; background: rgba(15, 15, 15, 0.95); padding: 30px; border-radius: 12px; text-align: center; color: #fff; box-shadow: 0 0 15px rgba(138, 43, 226, 0.2);">
          ...
          <div style="display: inline-block; background: rgba(255, 255, 255, 0.1); padding: 15px 30px; border-radius: 8px; backdrop-filter: blur(10px); font-size: 24px; font-weight: bold; color: #8A2BE2; letter-spacing: 2px; box-shadow: 0 0 10px rgba(138, 43, 226, 0.6);">
            ${otp}
          </div>
          <p style="margin-top: 20px; font-size: 16px; color: #bbb;">
            Click the button below to proceed:
          </p>
          <a href="${actionUrl}" style="display: inline-block; background: linear-gradient(90deg, #7F00FF, #E100FF); color: #fff; text-decoration: none; padding: 12px 30px; border-radius: 6px; font-size: 18px; font-weight: bold; box-shadow: 0 4px 10px rgba(127, 0, 255, 0.5);">
            ${actionText}
          </a>
