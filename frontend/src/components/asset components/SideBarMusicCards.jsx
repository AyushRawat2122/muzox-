...
const SideBarMusicCards = ({ className, onClick, Song }) => {
  return (
    <button className={`w-full hoverPlay`} onClick={onClick}>
      <div className="flex gap-2 p-2 items-center justify-between rounded-md bg-black/20 hover:bg-black/30 transition-all">
        <div className="flex gap-2">
          ...
          <div className="flex flex-col items-start capitalize">
            <h3
              className={`text-base font-semibold  h-5  overflow-hidden ${className}`}
            >
              {Song?.title}
            </h3>
            <p className="text-sm text-[#c9c9c9] h-5  font-medium overflow-hidden">
              {Song?.artist}
            </p>
          </div>
        </div>
